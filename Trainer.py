...
class Trainer:
    ...
    def preprocessing_data_incremental(chex_competition, xrays_position, basic_prompts, batch_size, lr,
        ...
        if epochs > 0:
            ...
            #     batch_size) + "-ep" + str(
            #     epochs) + chex_str + str_basic + "-" + str(xrays_position) + suffix + cl_str
            w_path = "NUOVI_RISULTATI/data-incremental/" + mode + "-loss-" + str(loss_name) + "-opt-" + OPTIM + "-lr-" + str(
                lr) + "-bs" + str(
                batch_size) + "-ep" + str(
                epochs) + "-parts" + str(
    ...
    def evaluate_model(self, y_true, y_pred, y_score, mode, epoch, val_test, epochs, tasks_order):
        ...
        if self.writer is not None:
            ...
            if val_test == "val":
                self.val_f1_heat_map, self.val_auroc_heat_map = self._aux_evaluate_model(y_true, y_pred, y_score, mode, epoch, val_test, epochs, tasks_order,
                                         self.val_f1_heat_map,
                                         self.val_auroc_heat_map)
            elif val_test == "test":
                self.test_f1_heat_map, self.test_auroc_heat_map = self._aux_evaluate_model(y_true, y_pred, y_score, mode, epoch, val_test, epochs, tasks_order,
